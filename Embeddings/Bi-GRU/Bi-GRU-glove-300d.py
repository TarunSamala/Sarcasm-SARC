...
# Configuration
MAX_LEN = 50               # Sequence length for comments
...
# Build the embedding matrix based on our tokenizer and the loaded GloVe embeddings
...
def build_bigru_with_glove():
    ...
    # Embedding layer with pre-trained weights, set trainable to True for fine-tuning.
    x = Embedding(VOCAB_SIZE, EMBEDDING_DIM,
                  weights=[embedding_matrix],
                  trainable=True,
                  embeddings_regularizer=regularizers.l2(1e-4))(inputs)
    
    # Slightly reduced dropout rate to preserve learned features
    x = SpatialDropout1D(0.4)(x)
    
    # Increased model capacity with more GRU units
    ...
    # Expanded Dense layer for improved feature learning
...
# Setup callbacks for early stopping and adaptive learning rate reduction
callbacks = [
...
# Save training curves: accuracy and loss plots
...
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, 'bigru_glove840_training_curves.png'), dpi=300)
plt.close()
...
with open(os.path.join(OUTPUT_DIR, 'bigru_glove840_classification_report.txt'), 'w') as f:
    f.write("Bi-GRU with glove.840B.300d.pkl Classification Report:\n")
    f.write(report)
...
plt.title('Bi-GRU with glove.840B.300d.pkl Confusion Matrix')
plt.ylabel('True Label')
plt.xlabel('Predicted Label')
plt.savefig(os.path.join(OUTPUT_DIR, 'bigru_glove840_confusion_matrix.png'), dpi=300)
plt.close()
